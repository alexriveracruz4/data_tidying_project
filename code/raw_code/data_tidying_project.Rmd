...
We have to set up by installing some packages and calling  the libraries that we have to use in this project.
...
Saving the data in ore raw_data directory in case the link disappear or we can't enter to the data in the future.
...
Wrangling the df2 data, passing from wide data to long data to get a dataframe with 3 columns(business,year, n)
...
Wrangling df1 data, shaping the data to get a dataframe with 3 columns (business,year,n)
...
Combining the two datasets in one dataset.

```{r merge-data, warning=FALSE} 
## Now, combine the two data frames
df_total <- combine(df_retail,df_department)
```

Plotting each business and the combined data through the years
